...
## Инструменты пост-анализа (tracing-analyzer)

В папке `tracing-analyzer` находится Java-утилита и Bash-скрипты для обработки сырых логов трассировки, собранных после прогона тестов.

- `run-tests.sh` — Автоматический прогон всех 140 фаззинг-тестов из корня репозитория `corax`.
- `generate-mapping.sh` — Запуск `ReleaseMappingGenerator.java` для сборки карты соответствия классов и JAR-файлов релиза.
- `analyze-traces.sh` — Запуск `TraceAnalyzer.java` для сопоставления сырых логов `.out` с картой классов и генерации финальных отчетов по покрытию методов.

Полная пошаговая инструкция по работе с анализатором и генерации отчетов вынесена в отдельный файл:
👉 **[Инструкция по анализу тестов (TESTNOTES.md)](TESTNOTES.md)**
